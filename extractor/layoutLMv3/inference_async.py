...
job_dir = os.path.join(current_dir, "results")
...
class layoutLMv3:
    def __init__(self, pil_queue, layout_queue) -> None:
        ...
        self.pil_queue = pil_queue
        self.layout_queue = layout_queue

    async def get_field_values(self):

        if not self.pil_queue.empty():
            image = await self.pil_queue.get()
            # Preprocess image
            ...
            for box, token, logits in zip(bboxes, tokens, outputs.logits[0]):
                ...
                if box != curr_box and segment_tokens:
                    ...
                    labels_for_unique_bboxes.append(
            ...
            for bbox, words, label in zip(
                ...
                font_size = max(int(box_height * 0.2), 12)
            ...
            await self.layout_queue.put(field_values)
        else:
            asyncio.sleep(1)
...
#     layoutLMv3_extractor = layoutLMv3(layout_queue)
#     await layoutLMv3_extractor.get_field_values(image)
#     field_values = None
#     if not layout_queue.empty():
